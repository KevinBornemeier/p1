...
//do not need to modify anything in WebServer.java.
//everytime we run 
//break 3 different values of GET /test.html HTTP/1.1
//use split commmand with string. Search how to use java to break string into seperate values
//only need the second (middle value).

//when reading and writing to socket, must convert message to byte before sending it through.
import java.net.*;
...
{
//declare object named workerSocket
   Socket workerSocket;
   WebWorker worker;
//WebWorker will do the work for you. WebServer handles the connection to the WebWorker.
   try {
   ...
   while (true) {
	 ...
	 // at this point the compiler will freeze.
	 //webserver will listen for request 
     ...
     //run as thread - means to run until stop operation is used.
